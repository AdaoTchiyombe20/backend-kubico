...
export const profileService = {
  createOwnerIndividual: async (
    ...
    try {
      ...
      const existingPersonProfile =
        await person_profilesRepository.findById(profile_id);

      if (existingPersonProfile) throw new AppError("Usuario ja existe", 400);

      await person_profilesRepository.createBase(profile_id, {
        full_name,
        birth_date,
        phone,
      });

      const existingRole = await profileRole.findProfileRoleByRole(
        profile_id,
        2,
      );
      ...
      await profileMediaRepository.insertMedia(
        profile_id,
        "CONTA_BANCARIA",
        bank_account,
      );
  ...
  createOwnerCompany: async (
    ...
    try {
      ...
      const existingCompany =
        await companyProfileRepository.findById(profile_id);

      if (!existingCompany) {
        await companyProfileRepository.createBase(profile_id, {
          legal_name,
          phone,
        });
      } else {
